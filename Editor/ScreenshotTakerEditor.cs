...
using UnityEngine;
using UnityEditor;
using Teamon.Tools.Screenshooter.Utils;
using static Teamon.Tools.Screenshooter.Utils.ReflectionEx;
#if POLYGLOT
using Polyglot;
#endif
...
namespace Teamon.Tools.Screenshooter
{
	public static class ScreenshotTakerEditor
	{
		...
		static object SizeHolder
		{
			get { return GetWindowType("GameViewSizes").FetchProperty("instance").FetchProperty("currentGroup"); }
		}

		static EditorWindow GameView
		{
			get { return EditorWindow.GetWindow(GetWindowType("GameView")); }
		}
	...
	#else
			EditorUtility.DisplayProgressBar("Making screenshots", $"{data.targetCamera} {width}x{height}",
				currentStage / (float) totalStages);
	#endif
			++currentStage;
		...
		{
			Camera camera = data.targetCamera == ScreenshooterTargetCamera.GameView
				? Camera.main
	...
	#else
				File.WriteAllBytes(
					ScreenshotTaker.GetUniqueFilePath(renderTex.width, renderTex.height,
						data.targetCamera == ScreenshooterTargetCamera.SceneView, false, "", usedOutputFolder, "jpeg"),
	...
	#else
				File.WriteAllBytes(
					ScreenshotTaker.GetUniqueFilePath(width, height,
						data.targetCamera == ScreenshooterTargetCamera.SceneView, true, "", usedOutputFolder, "jpeg"),
					screenshot.EncodeToJPG(100));
	#endif
			}
			finally
