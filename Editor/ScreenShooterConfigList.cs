...
namespace Teamon.Tools.Screenshooter
{
	...
	{
		public static UnityEditorInternal.ReorderableList Create(List<ScreenshotData> configsList,
			GenericMenu.MenuFunction2 menuItemHandler)
		{
			var reorderableList =
				new UnityEditorInternal.ReorderableList(configsList, typeof(ScreenshotData), true, false, true, true);
			...
			reorderableList.drawElementCallback = (position, index, isActive, isFocused) =>
			{
				...
				var nameWidth = position.width - space * 6 - enabledWidth - cameraWidth - textWidth - sizeWidth * 2 -
				                multWidth - uiWidth - singleWidth * 21;
			...
			reorderableList.onAddDropdownCallback = (buttonRect, list) =>
			{
				...
				foreach (var config in PredefinedConfigs.Android)
				{
					...
					else
					{
						...
						var label = "Android/" + altData.name + " (" + altData.resolution.x + "x" + altData.resolution.y +
						            ") (Landscape)";
				...
				foreach (var config in PredefinedConfigs.iOS)
				{
					...
					else
					{
						ScreenshotData altData = config;
						altData.resolution = new Vector2(config.resolution.y, config.resolution.x);
						var label = "iOS/" + altData.name + " (" + altData.resolution.x + "x" + altData.resolution.y +
						            ") (Landscape)";
						menu.AddItem(new GUIContent(label), false, menuItemHandler, altData);
					}
				}
